...
votes_migrants = pd.merge(votes_df, migrants_df, on=['country'])
...
model = smf.ols(formula = f'votes{int(YEAR) - 1} ~ votes{int(YEAR) - 2} + votes{int(YEAR) - 3} + votes{int(YEAR) - 4} \
                            + migrants{int(YEAR) - 1} + migrants{int(YEAR) - 2} + migrants{int(YEAR) - 3} + migrants{int(YEAR) - 4}\
                            + borders', data=country_merged.dropna())
model = model.fit()
print(model.summary())
